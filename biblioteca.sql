...
--
-- Servidor: 127.0.0.1
-- Tiempo de generación: 09-03-2026 a las 17:58:11
...
(765, 'HERRERA CARRILLO', '2001-06-26', '2025-05-03'),
...
ALTER TABLE `posiciones`
  ADD PRIMARY KEY (`id`),
  ADD UNIQUE KEY `ganados` (`ganados`),
  ADD KEY `jugados` (`jugados`);
...
-- Eventos
--
CREATE DEFINER=`root`@`localhost` EVENT `anual_eliminar_prestamos` ON SCHEDULE EVERY 1 YEAR STARTS '2026-03-09 06:40:42' ON COMPLETION NOT PRESERVE ENABLE DO BEGIN
    DELETE FROM tbl_prestamo
    WHERE pres_fechaDevolucion <= NOW() - INTERVAL 1 YEAR;
    #datos menores a la fecha actual - 1 año
END$$

CREATE DEFINER=`root`@`localhost` EVENT `ELIMINAR_PRESTAMOS` ON SCHEDULE EVERY 1 YEAR STARTS '2026-03-09 00:00:00' ENDS '2028-03-09 00:00:00' ON COMPLETION NOT PRESERVE ENABLE DO DELETE FROM prestamo
WHERE PRES_FECHADEVOLUCION <= NOW() - INTERVAL 1 YEAR$$
